  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (wilson_enabled) {
      ...
      for (int i = 0; i < wilson_offaxis_result.size(); i++) {
        ofstream_wilson << "0," << wilson_offaxis_result[i].time_size << ","
    ...
    if (HYP_enabled == 1) {
      ...
      for (int HYP_step = 0; HYP_step < HYP_steps; HYP_step++) {
        ...
        if (wilson_enabled) {
          ...
          for (int i = 0; i < wilson_offaxis_result.size(); i++) {
            ofstream_wilson
                << HYP_step + 1 << "," << wilson_offaxis_result[i].time_size
                << "," << wilson_offaxis_result[i].space_size << ","
                << wilson_offaxis_result[i].wilson_loop << std::endl;
          }
        }
    ...
    if (APE_enabled == 1) {
      ...
      for (int APE_step = 0; APE_step < APE_steps; APE_step++) {
        ...
        if (APE_step % calculation_step_APE == 0) {
          if (wilson_enabled) {
            wilson_offaxis_result = wilson_offaxis(conf.array, directions,
                                                   R_min, R_max, T_min, T_max);

            for (int i = 0; i < wilson_offaxis_result.size(); i++) {
              ofstream_wilson << APE_step + HYP_steps + 1 << ","
